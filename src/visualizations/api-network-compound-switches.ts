...
const pass1 = groupByUpstreamNode(raw.nodes, raw.edges, {
  ...
  minGroupSize: 1, // always nest a relay device under its known parent
});

const { nodes, edges } = groupByUpstreamNode(pass1.nodes, pass1.edges, {
  isLeaf: n => isHost(n) || isUnknownDevice(n),
  isUpstream: n => n.data.node_type === 'device', // any device, known or not
  minGroupSize: 1,
});
...
export function mount(container: HTMLElement): void {
  ...
  const ctrl = setupExpandCollapse(cy, nodes, edges, fcoseLargeProvider, NODE_HIERARCHY, 'switch', panelOpts);
